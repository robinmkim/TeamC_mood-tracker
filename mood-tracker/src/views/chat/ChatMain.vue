  ...
  <div class="flex h-screen">
    ...
    <div class="flex w-2/5 border-r border-gray-200 flex-col">
      ...
      >
        ...
        <input class="text-lg font-bold" v-model="chattingMember"/>
      </div>
...
export default {
    ...
    methods: {
        ...
        getMemberList() {
            apiClient
            .get("http://192.168.0.214:8083/follow/myfollow")
            .then((res) => {
        ...
        loadChatRooms() {
            ...
            apiClient
            .get(`http://192.168.0.214:8083/rooms`)
            .then((response) => {
                this.rooms = "";
        ...
        loadMessages(roomId, otherMemberName) {
            this.createNewSubscribe(roomId);
            this.chattingMember = otherMemberName;
            apiClient
        ...
        startNewChat(eventData) {
          ...
          const user = {
              userId1: this.memberId,
              userId2: newChatMemberId,
          };
          // 두 사용자 간 채팅방이 존재하는지 확인
          apiClient.post(`http://192.168.0.214:8083/rooms/check`, user)
              .then(response => {
                  // 채팅방이 존재하면 해당 채팅방으로 이동
                  if (response.data.length !== 0) {
                    this.loadMessages(response.data, newChatMemberName);
        ...
        createNewChatRoom(user) {
            apiClient.post('http://192.168.0.214:8083/rooms/new', user) 
                .then(response => {
